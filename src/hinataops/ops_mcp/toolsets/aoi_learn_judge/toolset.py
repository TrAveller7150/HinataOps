...
class AoiLearnJudgeToolset:
    ...
    def __init__(self, config: EnvironmentConfig) -> None:
        self._config = config
        runner = SshRunner(config.environment)
        ...
        self._docker = DockerReadonlyAdapter(
            runner, config.instance(config.aoi_judge.docker_instance_id, "docker")
        )
        self._mysql = MysqlReadonlyAdapter(
            runner, config.instance(config.aoi_judge.mysql_instance_id, "mysql")
        )
        self._prometheus = PrometheusReadonlyAdapter(
            runner, config.instance(config.aoi_judge.prometheus_instance_id, "prometheus")
        )
        redis_ids = {stream.redis_instance_id for stream in config.judge_streams}
        if len(redis_ids) != 1:
            raise ValueError("AoiLearn judge toolset currently requires one Redis instance")
        self._redis = RedisReadonlyAdapter(runner, config.instance(redis_ids.pop(), "redis"))

    async def get_container_runtime(self) -> DockerSnapshot:
        """返回拓扑服务与判题沙箱池的 Docker 状态。"""
        return await DockerInspector(
            self._config.environment.name, self._docker, self._config.services
        ).snapshot()
    ...
    async def get_stream_summary(self, language: JudgeLanguage) -> RedisStreamSummary:
        ...
        return await RedisStreamInspector(
            self._config.environment.name, self._redis, self._config.judge_streams
        ).summary(language)
    ...
    async def get_pipeline_summary(self, window_minutes: Literal[5, 15, 60]) -> MysqlJudgePipelineSummary:
        ...
        return await MysqlJudgePipelineInspector(
            self._config.environment.name, self._mysql
    ...
    async def get_runtime(self) -> PrometheusJudgeRuntimeSummary:
        ...
        return await PrometheusJudgeRuntimeInspector(
            self._config.environment.name, self._prometheus, self._config.judge_streams
        ).summary()
